...
#  Add var to choose between an existing Artifact and an inline one
...
resource "oci_devops_deploy_artifact" "test_deploy_oke_artifact" {
  ...
  deploy_artifact_source {
    ...
    base64encoded_content       = templatefile("${path.module}/manifest/queue-fn-automation.yaml", { ocir_repo = "${local.ocir_docker_repository}", name = "${local.ocir_namespace}", image = "${oci_artifacts_container_repository.test_container_repository.display_name}", hash = "$${BUILDRUN_HASH}", namespace = "$${namespace}", queue_region=var.queue_region, function_region=var.function_region, queue_function_map=local.queue_function_map})
  }
  defined_tags = { "${oci_identity_tag_namespace.ArchitectureCenterTagNamespace.name}.${oci_identity_tag.ArchitectureCenterTag.name}" = var.release }
}
